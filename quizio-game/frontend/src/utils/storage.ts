...
export const storage = {
    ...
    appFontSize: {
        get: (): 'normal' | 'large' | 'xlarge' | null =>
            localStorage.getItem(KEYS.APP_FONT_SIZE) as
                | 'normal'
                | 'large'
                | 'xlarge'
                | null,
        set: (v: 'normal' | 'large' | 'xlarge') =>
            localStorage.setItem(KEYS.APP_FONT_SIZE, v),
    ...
    setupData: {
        ...
        set: (v: SetupData) =>
            localStorage.setItem(KEYS.SETUP_DATA, JSON.stringify(v)),
        clear: () => localStorage.removeItem(KEYS.SETUP_DATA)
    },
    studentCreds: {
        get: (): StudentCreds | null =>
            parseJson<StudentCreds>(KEYS.STUDENT_CREDS),
        set: (v: StudentCreds) =>
            localStorage.setItem(KEYS.STUDENT_CREDS, JSON.stringify(v)),
