...
DATA_SERVICE_URL_BASE = os.getenv('DATA_SERVICE_URL_BASE', 'http://host.docker.internal:8080')
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"❌ 學生驗證失敗: 回傳狀態碼 {response.status_code} - {response.text}")
                return None
    ...
    async with httpx.AsyncClient() as client:
        try:
            response = await client.post(
                ...
                data={'username': DATA_SERVICE_ADMIN_USERNAME, 'password': DATA_SERVICE_ADMIN_PASSWORD},
                timeout=5.0
            )
